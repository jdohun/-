...
이 부분은 디버깅을 위해 넣어놓은 부분이므로 제거하였습니다.


dijkstra에서는 D 매트릭스는 인접행렬로 사용하지 않고 한행만 사용합니다.

D[j]는 노드0에서 노드j에 이르는 최단경로의 길이를 나타냅니다.
*/
...
void dijkstra(int n, int* W, int* D)
{
    int* nearest;
    int* distance;
    nearest = new int[n];
